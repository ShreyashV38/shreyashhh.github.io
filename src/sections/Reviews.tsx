...
export default function Reviews() {
  ...
  const handleLike = (reviewId: number) => {
    posthog?.capture("review_liked", { review_id: reviewId });
        ...
        <div className="reveal mb-16">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-bold tracking-tight mb-4">
            ...
            <span
              ...
              style={{ textShadow: "0 0 10px rgba(245,216,0,0.3)" }}
            >
              REVIEWS
            </span>
          </h2>
                    ...
                    {[1, 2, 3, 4, 5].map(star => (
                      <button
                        ...
                        className={`text-lg transition-all duration-200 cursor-hover ${
                          star <= rating
                            ? "text-[#F5D800] drop-shadow-[0_0_6px_rgba(245,216,0,0.5)]"
                            : "text-[#8A9BA8]/20"
                        }`}
                      >
                        ★
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
            {reviews.length === 0 ? (
              <div className="reveal glass-panel border border-[#8A9BA8]/10 p-8 text-center">
                <p className="font-terminal text-sm text-[#8A9BA8]/40">
                  No feedback entries. Be the first to submit a review.
                </p>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="font-body text-sm font-semibold text-[#F0F0F0]">
                          {review.name}
                        </div>
                        {review.role && (
                          <div className="font-terminal text-[10px] text-[#8A9BA8]/40">
                            {review.role}
                          </div>
                        )}
                      </div>
                      <div className="flex gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <span
                            key={i}
                            className={`text-[10px] ${i < review.rating ? "text-[#F5D800]" : "text-[#8A9BA8]/15"}`}
                          >
                            ★
                          </span>
                        ))}
                    ...
                    <p className="font-body text-[13px] text-[#8A9BA8] leading-relaxed mb-3">
                      {review.comment}
                    </p>
                    <div className="flex items-center justify-between pt-3 border-t border-[#8A9BA8]/10">
                      <button
                        ...
                        className={`flex items-center gap-2 font-terminal text-[10px] tracking-[1px] transition-all duration-200 cursor-hover ${
                          isLiked
                            ? "text-[#00E5FF]"
                            : "text-[#8A9BA8]/30 hover:text-[#00E5FF]"
                        }`}
                      >
                        {isLiked ? "▣" : "▢"} {review.likes}
                      </button>
                      <span className="font-terminal text-[10px] text-[#8A9BA8]/20">
